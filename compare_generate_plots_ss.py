...
idsort = np.argsort(seff)

err = acts[idsort].squeeze() - preds[idsort].squeeze()
plt.scatter(seff[idsort],err,s=1,color='r',alpha=1.0)
...
# plt.show()
fig.tight_layout()
# plt.savefig('images/ss_sort.png', dpi=300)
plt.close()
...
binedges = np.quantile(seff,pvals)
...
idbins = np.digitize(seff,binedges)
...
plt.xlabel(r"Supersaturation bins")
plt.ylabel("Error")
fig = plt.gcf()
# fig.tight_layout()
# plt.savefig('images/ssbins.png', dpi=300)
# plt.show()
# plt.close()
...
ax_inset.scatter(seff[idsort],err,s=1,color='r',alpha=1.0,label=r'$s_{fil}$')
...
fig.tight_layout()
plt.savefig('images/compare_qtiles_bins.png', dpi=300)
plt.close()
...
# nbinedges = 6
# # we extend the last binedge to beyont the maximum value
# binedges = np.linspace(1.00001*min(seff),1.00001*max(seff),nbinedges)
# idbins = np.digitize(seff,binedges)
...
# fig.set_size_inches(16, 8)  # width: 16 inches, height: 8 inches
# fig.tight_layout()
# plt.savefig('images/ss_uniform_bins.png', dpi=300)
# plt.close()
